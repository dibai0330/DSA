...
void ExtendTree(Node* root){
    ...
    if(root->left ==NULL)   root->left = CreateNode('#', NULL, NULL);
    else if(root->left !=NULL)  ExtendTree(root->left);

    if(root->right ==NULL)  root->right = CreateNode('#', NULL, NULL);
    else if(root->right !=NULL)  ExtendTree(root->right);
}
